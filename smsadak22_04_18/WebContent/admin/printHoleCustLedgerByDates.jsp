...
<%@ page language="java" import="dTO.CustomerLedgerDto"%>
<html>
<head>
...
<body onload="noBack();" onpageshow="if (event.persisted) noBack();"
	...
	<div class="wrapper row-offcanvas row-offcanvas-left">
		...
		<h1>
		SALE LEDGER <small> print hole customer ledger details <img src="img/ledger.png" height="60px" width="60px" style="margin: -14px -22px -21px 0px"/></small>
		</h1>
		<ol class="breadcrumb">
			<li><a href="adminIndexCall.action">Home</a></li>
			<li class="active">Print hole sale Ledger</li>
		</ol>
		</section>
			...
			<div class="row">
				<div class="LongTable">
					...
					<div style="height: 50px; margin: 50px -8px -11px 20px">
						<label for="SaleDate">Select Customer:</label>
						<s:select headerKey="0" onchange="#" onmouseover="called()"
							...
							listKey="customerId" listValue="cName"
							 id="custID" theme="simple" />
					</div>
					...
					<div class="box box-primary">
						<div style="height: 50px; margin: -44px 11px -3px 432px">
							...
							Due Date: <input type="text" class="textboxStockloc" id="endDate" name="fromdate" />
							
							<input type="hidden" name="fYearS" id="fYearS" value="${sessionScope['fYear']}" >
							
							<img src="img/searchDate.png" width="45px" height="45px"
								style="margin: -10px 43px -17px 16px; cursor: pointer;"
								onclick="searchCustHoleLedgerByDATE('<%=request.getContextPath()%>',document.getElementById('startDate').value,document.getElementById('endDate').value)" />
						</div>
						<div id="purchaseLedgerDetails" style="height: 800px">
							
							<embed src="<%=request.getContextPath() %>/ledgerReports/customerHoleledger.pdf" width="100%" height="900px"></embed>
						</div>
					</div>
				</div>
			</div>
