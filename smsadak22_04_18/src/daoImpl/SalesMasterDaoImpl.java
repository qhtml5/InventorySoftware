...
public class SalesMasterDaoImpl extends ConnectCore implements SalesMasterDao {
	...
	public List<CustomerMasterDTO> getAllRegisterCustomerNames() throws DaoException, SQLException {
		List<CustomerMasterDTO> list = new LinkedList<CustomerMasterDTO>();
		Connection con = null;
		PreparedStatement pstat=null;
		String query;
		...
		try {
			...
			cid=(String)session.getAttribute("companyId");
			cid=cid.trim();
			query = "SELECT * FROM customermaster WHERE cName<> 'CASH' AND cName<>'NA'";
...
//				saleSql="SELECT b.billNo,b.challanNo,b.ldate,b.itemId,i.itemname,b.itemSize,b.qty,b.buyrate,b.supplierId,s.SUPPLIERNAME,b.itemTot,b.cst,b.waybill,b.vat FROM purchaseledgermaster b JOIN itemmaster i ON b.itemId=i.itemId JOIN suppliermaster s ON b.supplierId=s.SupplierId where b.ldate between '"+fromdate+"' And '"+todate+"' And b.companyId='"+CurrentCompAndUser.getCurrentCompany()+"'  AND b.cst<>'0.00' ORDER BY b.ldate ASC";	
		...
		while(rs.next())
		{
			...
			double gstamt=basetot * 5/100;
			
			pd.setGstrate(rs.getString("gstrate"));
			pd.setVat(rs.getString("gsttin"));
			pd.setGstamt(new DecimalFormat("##.##").format(gstamt)+"");
			pd.setCgstrate(rs.getString("cgstrate"));
			pd.setCgstamt(new DecimalFormat("##.##").format(gstamt/2)+"");
			pd.setSgstrate(rs.getString("sgstrate"));
			pd.setSgstamt(new DecimalFormat("##.##").format(gstamt/2)+"");
			fbasetot=fbasetot+basetot;
		...
		System.out.println("Size of List found... " +list.size());

		
		
		
		int incrementedsize=list.size()+1;
		for(int l=list.size();l<incrementedsize;l++)
		{
			SalesMasterDto pd=new SalesMasterDto();
		...
		int incrementedsize2=list.size()+1;
		for(int l=list.size();l<incrementedsize2;l++)
		{
			SalesMasterDto pd=new SalesMasterDto();
			
			double gstamt=0.0;
			gstamt=fbasetot*5/100;
			
			double finalgstamt=fbasetot+gstamt;

			pd.setBillno("TOTAL");
			pd.setDoc("");
			pd.setCustomerName("");
			pd.setDoc("");
			pd.setGrandTotal(finalgstamt+"");
			pd.setGstrate(list.get(0).getGstrate());
			pd.setVat("" );
			pd.setGstamt(gstamt+"");
			pd.setCgstrate(list.get(0).getCgstrate());
			pd.setCgstamt(gstamt/2+"");
			pd.setSgstrate(list.get(0).getSgstrate());
			pd.setSgstamt(gstamt/2+"");
			pd.setBasetotal(fbasetot+"");
			list.add(pd);	
...
//				pstat.close();
	...
	public List<SalesMasterDto> exportToexcelSaleDetailsgstr(String fromdate, String todate,String customerId,String cstvat, String CNorNoCN ) throws DaoException, SQLException 
	{
		...
		else if(Integer.parseInt(customerId)>0 && !fromdate.equals("") && !todate.equals("")&& cstvat.equals(""))
		{
			...
			saleSql="SELECT * FROM salemaster WHERE saleDoc BETWEEN '"+fromdate+"' AND '"+todate+"' AND regCustomerId='"+customerId+"' GROUP BY billno ORDER BY billno ASC";
			flagforTitle=2;
		}
		else if(Integer.parseInt(customerId)>0 && fromdate.equals("") && todate.equals("")&& !cstvat.equals(""))
		{
...
//			pd.setBillno(rs.getString("billno"));
			...
			{
				totgstamt=totgstamt+Double.parseDouble(s.getGstamt());
				totcgstamt=totcgstamt+Double.parseDouble(s.getCgstamt());
				totsgstamt=totsgstamt+Double.parseDouble(s.getSgstamt());
				totIgstamt=totIgstamt+Double.parseDouble(s.getIgstamt());
				basetot=basetot+Double.parseDouble(s.getBasetotal());
			}
			...
			{
				pd.setTitleforExcel("TOTAL SALE  DETAILS FROM CUSTOMER  "+getCustomerName(rs.getString("regcustomerId").toString(), con)+" FROM DATE "+fromdate+" TO DATE "+todate);	
			}
			else if(flagforTitle==3)
			{
				pd.setTitleforExcel("TOTAL SALE  DETAILS FROM CUSTOMER  "+rs.getString("cName") +" FROM DATE "+fromdate+" TO DATE "+todate);	
...
//				pd.setBillno(rs.getString("billno"));
				...
				pd.setSgstamt(rs.getString("sgstamt"));
				 ...
				 fbasetot=Double.parseDouble(getBaseTotal(rs.getString("billno").toString(), con));
				...
				{
					pd.setTitleforExcel("TOTAL SALE  DETAILS FROM CUSTOMER  "+getCustomerName(rs.getString("regcustomerId").toString(), con));	
				}
				else if(flagforTitle==3)
				{
					pd.setTitleforExcel("TOTAL SALE  DETAILS FROM CUSTOMER  "+getCustomerName(rs.getString("regcustomerId").toString(), con) +" FROM DATE "+fromdate+" TO DATE "+todate);	
...
//				pstat.close();
	...
	public List<SalesMasterDto> exportToexcelSaleDetailsgstrexpnopan(String fromdate, String todate,String customerId,String cstvat, String CNorNoCN ) throws DaoException, SQLException 
	{
		List<SalesMasterDto> list = new ArrayList<SalesMasterDto>();
		Connection con = null;
		PreparedStatement pstat=null;
...
//				saleSql="SELECT b.billNo,b.challanNo,b.ldate,b.itemId,i.itemname,b.itemSize,b.qty,b.buyrate,b.supplierId,s.SUPPLIERNAME,b.itemTot,b.cst,b.waybill,b.vat FROM purchaseledgermaster b JOIN itemmaster i ON b.itemId=i.itemId JOIN suppliermaster s ON b.supplierId=s.SupplierId where b.ldate between '"+fromdate+"' And '"+todate+"' And b.companyId='"+CurrentCompAndUser.getCurrentCompany()+"'  AND b.cst<>'0.00' ORDER BY b.ldate ASC";	
		...
		while(rs.next())
		{
			if(checkCNorNotForBillNo(rs.getString("billno"), con)==false)
			{
				String gsttinc=checkcustomerPanNOTExist(rs.getString("regcustomerId"), con);
				if(!gsttinc.equalsIgnoreCase(""))
				{
...
//					dto.setSalesId(rs.getString("salesId"));
	...
	public List<SalesMasterDto> exportToexcelSaleDetailsgstrexpwithpan(String fromdate, String todate,String customerId,String cstvat, String CNorNoCN ) throws DaoException, SQLException 
	{
		List<SalesMasterDto> list = new ArrayList<SalesMasterDto>();
		Connection con = null;
		PreparedStatement pstat=null;
		...
		while(rs.next())
		{
			if(checkCNorNotForBillNo(rs.getString("billno"), con)==false)
			{
				...
				System.out.println("Customer registration number found"+rs.getString("regcustomerId"));
				
		
				
				String gstinnofound=checkcustomerPanExistreturngstno(rs.getString("regcustomerId"), con);
				
				
				if(!gstinnofound.equalsIgnoreCase(""))
				{
...
//				pstat.close();
	...
	public boolean checkcustomerPanExist(String customerid,Connection con)throws DaoException ,SQLException
	{
		boolean rst=false;

		
//		String query="SELECT * FROM customermaster WHERE gsttin  LIKE'%PAN%' OR  gsttin LIKE'%AAD%' OR gsttin  LIKE'%VO%' OR gsttin NOT LIKE'%19%'  AND customerId='"+customerid+"'";
		
		
		String query="SELECT * FROM customermaster WHERE  customerId='"+customerid+"'";
		...
		try
		{
			...
			while(rs.next())
			{
				String val=rs.getString("gsttin");
				
				System.out.println("GST SUB++++++++++++++++++++++++ "+val.substring(0,2));
				
				if(val.substring(0,2)!="19")
				{
	...
	public String checkcustomerPanExistreturngstno(String customerid,Connection con)throws DaoException ,SQLException
	{
		String rst="";
	...
	public String checkcustomerPanNOTExist(String customerid,Connection con)throws DaoException ,SQLException
	{
		String rst="";

		
//		String query="SELECT * FROM customermaster WHERE gsttin NOT LIKE '%PAN%' AND gsttin  NOT LIKE'%AAD%' AND gsttin  NOT LIKE'%VO%' AND customerId='"+customerid+"'";
		String query="SELECT * FROM customermaster WHERE  customerId='"+customerid+"'";
		System.out.println("query found"+query);
	...
	public List<SalesMasterDto> exportHSNWISEREPORT(String fromdate, String todate,String customerId,String cstvat, String CNorNoCN ) throws DaoException, SQLException 
	{
		List<SalesMasterDto> list = new ArrayList<SalesMasterDto>();
		Connection con = null;
		PreparedStatement pstat=null;
...
//		saleSql="SELECT * FROM salemaster WHERE saleDoc BETWEEN '"+fromdate+"' AND '"+todate+"' GROUP BY billno ORDER BY billno ASC";
		...
		saleSql="SELECT DISTINCT hsno  FROM salemaster WHERE   saleDoc BETWEEN '"+fromdate+"' AND '"+todate+"' AND hsno<>''";
		...
		while(rs.next())
		{
		    ...
		    pd.setHsncode(rs.getString("hsno"));
			String searchhnoQuery="SELECT * FROM salemaster WHERE hsno='"+rs.getString("hsno").toString()+"' AND saleDoc BETWEEN '"+fromdate+"' AND '"+todate+"'";
			
			PreparedStatement pssearch=con.prepareStatement(searchhnoQuery);
			ResultSet rssearch=pssearch.executeQuery();
			while(rssearch.next())	
			{
			totqtysold=totqtysold+Double.parseDouble(rssearch.getString("saleQty"));	
//			totalvalue=totalvalue+Double.parseDouble("saleItemtotal");		
			taxablevalue=taxablevalue+Double.parseDouble(rssearch.getString("basetotal"));		
				
			}
			
			gstamt=taxablevalue*gstrate/100;
			cgst=gstamt/2;
			sgst=gstamt/2;
			totalvalue=taxablevalue+gstamt;
			
			pd.setCgstamt(new DecimalFormat("##.##").format(cgst)+"");
			pd.setSgstamt(new DecimalFormat("##.##").format(sgst)+"");
			pd.setQty(new DecimalFormat("##.##").format(totqtysold)+"");
			pd.setItemtotal(new DecimalFormat("##.##").format(totalvalue)+"");
			pd.setBasetotal(new DecimalFormat("##.##").format(taxablevalue)+"");
